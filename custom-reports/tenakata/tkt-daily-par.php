...
while($aa = mysqli_fetch_array($all_addons)){
    ...
    if(in_array($addon_id, array(1, 5, 14, 15, 16, 21, 24))) {
        ////-----Interest
        $interest_addons[$lid] = $addon_amount;
    }
    elseif (in_array($addon_id, array(3, 23))){
        ////----Membership
        $membership_addons[$lid] = $addon_amount;
    }
    elseif (in_array($addon_id, array(2, 22))){
        ////----Processing
        $processing_addons[$lid] = $addon_amount;
    }
    elseif (in_array($addon_id, array(4, 7, 18, 19, 20))){
        ////---All penalties
...
while($l = mysqli_fetch_array($loans_d)){
    ...
    $loans_totals_array = obj_add($loans_totals_array, $given_date, $lamount);
    $repaid_totals_array = obj_add($repaid_totals_array, $given_date, $repaid);
    $repayable_total_array = obj_add($repayable_total_array, $given_date, $total_repayable);

    $interest_totals_array = obj_add($interest_totals_array, $given_date, $loan_int);
    $penalty_totals_array = obj_add($penalty_totals_array, $given_date, $penalties);
    $daily_penalty_totals_array = obj_add($daily_penalty_totals_array, $given_date, $daily_penalties);
    $processing_totals_array = obj_add($processing_totals_array, $given_date, $processing);
    $membership_totals_array = obj_add($membership_totals_array, $given_date, $membership);
    $addons_total_array = obj_add($addons_total_array, $given_date, $total_addons);

    if($status == 7){
        $loan_balance = $l['loan_balance'];
        $defaulted_array = obj_add($defaulted_array, $given_date, $loan_balance);
    }
...
<?php
function generateDateRange($start_date_str, $end_date_str) {
    ...
    // Iterate over the DatePeriod and store the date strings in the array
    foreach ($date_range as $date) {
        $date_array[] = $date->format('Y-m-d');
    }

    // Include the end date in the array
    $date_array[] = $end_date->format('Y-m-d');
