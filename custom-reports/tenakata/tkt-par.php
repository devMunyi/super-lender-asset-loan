...
echo "<thead><tr><th>Branch</th><th>Principal</th><th>Interest</th><th>Membership</th><th>Processing</th><th>Penalties</th><th>DailyInterest (DD+31-45)</th><th>Total Repayable</th><th>Loan Payments</th><th>Balances</th><th>Default</th><th>Collection Rates</th><th>~Interest Rate</th></tr></thead>";
?>
<tbody>
<?php
...
foreach ($branches as $bid => $bname) {
    ...
    echo "<tr><td>$bname</td><td>".number_format($amt)."</td><td>".number_format($int_amount)."</td><td>".number_format($membership_totals)."</td><td>".number_format($processing_amount)."</td><td>".number_format($penalty_amount)."</td><td>".number_format($daily_penalty_amount)."</td><td>".number_format($total_rep)."</td><td>".number_format($repaid_pi)."</td><td>".number_format($bal)."</td><td>".number_format($defaulted)."</td><td>$crate%</td><td>$interest_rate%</td></tr>";
