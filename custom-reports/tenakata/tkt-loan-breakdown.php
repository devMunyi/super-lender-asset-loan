...
$interests = array(1,5,8,14,16,15,17,21,24);
$registrations = array(3,23);
$processings = array(2,22);
$penalties = array(4,6,7,18,19,20);
...
echo "<thead><tr><th>UID</th><th>Phone</th><th>Branch</th><th>Product</th><th>Principal Applied</th><th>Principal Paid</th><th>Principal Balance</th><th>Interest Applied</th><th>Interest Paid</th><th>Interest Balance</th><th>P. Applied + I. Applied (Repayable)</th><th>P. Paid + I. Paid (Repaid)</th><th>P+I Repayment Balance </th><th>Registration Fees Applied</th><th>Registration Fees Paid</th><th>Processing Fees Applied</th><th>Processing Fees Paid</th><th>Other Charges Applied(Penalties, Court etc)</th><th>Other Charges Paid</th><th>Other Charges Balance</th><th>Client Total Repaid (All)</th><th>Total Balance (All Outstanding)</th><th>Disbursed Date</th><th>Due Date</th><th>Cleared Date</th><th>Last Pay Date</th><th>Status</th></tr></thead>";


?>
<tbody>
<?php
$loans = fetchtable('o_loans',"disbursed=1 AND status!=0 AND given_date >= '$start_date' AND given_date <='$end_date' $andbranch_loan","uid","asc","100000000","uid, customer_id, account_number, loan_amount, total_repayable_amount, total_repaid, loan_balance, total_addons, given_date, current_lo, current_co, total_addons,current_branch, status, final_due_date, cleared_date, status, product_id");
while($l = mysqli_fetch_array($loans)){
    ...
    $processing_fees_applied = $processing_fees;
    $membership_fees_applied = $membership_fees;
    /////----------
    $principle_paid = false_zero($total_repaid - $upfronts);  ////Deduct from amount paid first
    if($principle_paid > $loan_amount){
    ...
    ///----Check if membership fee and processing fee is paid
    if($membership_fees > $total_repaid){
        $membership_fees = $total_repaid;
    }
    if($processing_fees > ($total_repaid - $membership_fees)){
        $processing_fees = false_zero($total_repaid - $membership_fees);
    }

    ////

    $other_charges_applied = false_zero($penalties);
    ...
    echo "<tr><td>$uid</td><td>$account_number</td><td>$branch_name</td><td>$product_name</td><td>".number_format($loan_amount)."</td><td>".number_format($principle_paid)."</td><td>".number_format($principle_balance)."</td><td>".number_format($interest)."</td><td>".number_format($interest_paid)."</td><td>".number_format($interest_balance)."</td><td>".number_format($pi_repayable)."</td><td>".number_format($pi_repaid)."</td><td>".number_format($pi_balance)." </td><td>".number_format($membership_fees_applied)."</td><td>".number_format($membership_fees)."</td><td>".number_format($processing_fees_applied)."</td><td>".number_format($processing_fees)."</td><td>".number_format($other_charges_applied)."</td><td>".number_format($other_charges_paid)."</td><td>".number_format($other_charges_balance)."</td><td>".number_format($total_repaid)."</td><td>".number_format($loan_balance)."</td><td>$given_date</td><td>$final_due_date</td><td>$cleared_date</td><td>$last_pay_date</td><td>$state</td></tr>";
