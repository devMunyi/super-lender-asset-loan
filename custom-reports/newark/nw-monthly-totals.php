...
$loan_l  = table_to_array('o_loans',"disbursed=1 AND status!=0 AND given_date >= '$start_date' AND given_date <= '$end_date' $andbranch_loans","10000000","uid","uid","asc");
...
$loans_d = fetchtable('o_loans',"disbursed=1 AND status!=0 AND given_date >= '$start_date' AND given_date <= '$end_date' $andbranch_loans","uid","asc","100000000000","uid, loan_amount, given_date, total_repaid, total_addons, status ,total_repayable_amount, loan_balance");
...
echo "<thead><tr><th>Year</th><th>Month</th><th>Principal</th><th>Interest</th><th>Membership</th><th>Processing</th><th>Penalties</th><th>DailyInterest (DD+31-45)</th><th>Total Repayable (All)</th><th>Total Repayable (P+I)</th><th>Collections (All)</th><th>Loan Payments (P+I)</th><th>Balances(All)</th><th>Default</th><th>Collection Rates(P+I)</th><th>~Interest Rate</th></tr></thead>";
?>
<tbody>
<?php
...
echo "*Interest: Interest applied to the loan so far 20%-25% depending on the day the loan was given or cleared<br/>";
echo "*Penalties: All the penalties charged on the Loan so far<br/>";
echo "*Total Repayable (All): Principal+Interest accrued + Penalties + Fees + Daily Interest<br/>";
echo "*Total Repayable (P+I): Principal+Interest accrued<br/>";
echo "*Collections (All): All monies that came in during the period, regardless of the loans<br/>";
echo "*Loan Payments(P+I): Payments that have been paid towards Principal and interest. It's calculated by taking all the incoming amount from a customer and deducting all penalties and fees. For example, if a client pays 1000 and has a penalty of 200 and late penalty of 100. Loan repayment will be 700 (1000-200-100)<br/>";
echo "*Balances: Full loan balance. Total Repaid amount - (Principal+Interest+Penalties+Processing+Admin+Registration)<br/>";
echo "*Default(All): Amount that has gone overdue (Includes Penalties, Processing , Admin, Registration)<br/>";
echo "*Collection Rate(P+I): The rate of collection for Principal+Interest Only. (P+I Collected / P+I Charged)*100 <br/>";
echo "*~Interest Rate: Average interest rate. With what average are the loans being paid. Calculated as (Total Interest Rate/Number of Loans)<br/>";
echo "</div>";
