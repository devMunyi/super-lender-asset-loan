...
if($bdo > 0){
    $andbdo_loan = " AND (current_lo=$bdo OR current_co=$bdo)";
}
else{
    $andbdo_loan = "";
}
...
<tbody>
<?php
$loans = fetchtable('o_loans',"disbursed=1 AND status!=0 AND given_date >= '$start_date' AND given_date <='$end_date' $andbdo_loan","uid","asc","100000000","uid, customer_id, account_number, loan_amount, total_repayable_amount, total_repaid, loan_balance, total_addons, given_date, current_lo, current_co, total_addons,current_branch, status, final_due_date");
while($l = mysqli_fetch_array($loans)){
    ...
    $deficit_total+=false_zero($deficit);
    $membership_total+=$membership_fees;


    echo "<tr><td>$uid</td><td>$name</td><td>$account_number</td><td>$branch_name</td><td>".money($loan_amount)."</td><td>".money($interest)."</td><td>".money($interest_principal)."</td><td>$given_date</td><td>$days_ago</td><td>".money($daily_expected)."</td><td>".money($principle_interest_paid)."</td><td class='font-bold text-purple'>".money(false_zero($deficit))."</td><td>$lo_co</td></tr>";
