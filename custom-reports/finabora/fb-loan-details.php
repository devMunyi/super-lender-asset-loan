<?php
$loansq = "disbursed = 1 AND status != 0  $andbranch_loan $andLoanProductId AND given_date BETWEEN '$start_date' AND '$end_date'";
...
while($ad = mysqli_fetch_array($ado)){
    ...
    ///------------Interests 
    if(in_array($addon_id, [1, 3, 4, 6, 7, 11])){
    ...
    /// ------------Penalties
    if(in_array($addon_id, [5, 8, 9, 10, 12, 13])){
