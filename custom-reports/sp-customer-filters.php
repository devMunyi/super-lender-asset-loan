...
} else if ($typ == 'DORMANT-CUSTOMERS') {
    echo "<h3>DORMANT CUSTOMERS</h3>";
    ///===== Begin Dormant Customers
...
";
    ...
    $dormant_customers = mysqli_query($con, $query2);
    $dormant_cust_branches = [];
    // $dormant_customer_array = [];
    $dormant_at_kv = [];
    while ($row = mysqli_fetch_assoc($dormant_customers)) {
        $cust_id = $row['uid'];
        $branch_id = $row['branch'];
        $dormant_customer_array[] = $cust_id;
        $dormant_at_kv[$cust_id] = $row['dormant_at'];
        // $dormant_cust_branches = obj_add($dormant_cust_branches, $branch_id, 1);
    }

    $the_list = implode(',', $dormant_customer_array);

    //////============End of Dormant customers
...
<table class="table table-condensed table-striped" id="example2">
    <thead>
        <tr>
            ...
            <th>Phone</th>
            <th>Agent</th>
            <th><?php echo $type == 'DORMANT-CUSTOMERS' ? 'Dormant At' : 'Added Date' ?></th>
            <th>Status</th>
            <th>Action</th>
        </tr>
    </thead>
    <tbody>
        ...
        while ($b = mysqli_fetch_array($b_leads)) {
            ...
            if ($typ == 'DORMANT-CUSTOMERS') {
                $added_date = $dormant_at_kv[$buid];
            }
            $status = $b['status'];
