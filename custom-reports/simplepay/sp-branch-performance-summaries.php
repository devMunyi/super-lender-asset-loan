...
///////-----------------------Disbursements
$loans = fetchtable('o_loans',"disbursed=1 $andbranch_loan AND given_date >= '$start_date' AND given_date <= '$end_date' AND status!=0","uid","asc","100000000","uid, customer_id ,loan_amount,  current_branch, current_lo, current_co, total_repaid, loan_balance, product_id");
$branch_totals_array = array();
$branch_balances_array = array();
...
while($dp = mysqli_fetch_array($loans)){
    ...
    /////-------Disbursed
    $branch_totals_array = obj_add($branch_totals_array, $current_branch, $loan_amount);$current_co_array = obj_add($current_co_array, $current_co, $total_repaid); 
    $current_lo_array = obj_add($current_lo_array, $current_lo, $loan_amount);
    $product_disb_array = obj_add($product_disb_array, $product_id, $loan_amount);
    $product_coll_array = obj_add($product_coll_array, $product_id, $total_repaid);
...
//////////////////---------------------------------Dues
$loans_ = fetchtable('o_loans',"disbursed=1 $andbranch_loan AND final_due_date >= '$start_date' AND final_due_date <= '$end_date' AND status!=0","uid","asc","100000000","uid, customer_id,  current_branch, loan_balance");
$branch_balances_array = array();

while($dp_ = mysqli_fetch_array($loans_)){
    $loan_id = $dp_['uid'];
    $current_branch = $dp_['current_branch'];

    $loan_balance = $dp_['loan_balance'];


    ///------Balances
    // if ($branch_balances_array[$current_branch] > 0) {
    //     $branch_balances_array[$current_branch] = $loan_balance + $branch_balances_array[$current_branch];
    // } else {
    //     $branch_balances_array[$current_branch] = $loan_balance;
    // }

    $branch_balances_array = obj_add($branch_balances_array, $current_branch, $loan_balance);
...
while($col = mysqli_fetch_array($collections)){
    ...
    $branch_totals_collections_array = obj_add($branch_totals_collections_array, $current_branch, $collections_amount);
...
echo "<table class='table table-bordered grid-width-50 col-lg-6' style='width: 40%;'>";
    echo "<thead><tr><th>Branch</th><th>Disbursement</th><th>Collections</th><th>Outstanding</th></tr></thead>";
    ...
    $balances = $branch_balances_array[$bid];
    ...
    $balances_total = $balances_total + $balances;
    echo "<tr><td>$bname</td><td>".money($branch_total)."</td><td>".money($collection_branch)."</td><td>".money($balances)."</td></tr>";
    }

    echo "<thead><tr><th>Total</th><th>".money($disb_total_total)."</th><th>".money($collections_total)."</th><th>".money($balances_total)."</th></tr></thead>";
    echo "</table>";
?>
<div class="col-md-6 well well-sm">
    <?php
    for($i=1; $i<=$thismonth; ++$i){
        echo "<a class='btn btn-outline-black' href=\"?hreport=sp-branch-performance-summaries.php&hid=$hid&m=$i\">".month_name($i)."</a> ";
    }
    if(isset($_GET['m'])){
        $m = $_GET['m'];
    }
    ...
    $due_ = fetchtable('o_loans',"disbursed=1 $andbranch_loan AND given_date BETWEEN '$thisyearmonth_last_month' AND '$thisyearmonthend'","uid","asc","1000000","uid, loan_amount, total_repayable_amount, total_repaid, current_branch");
    while ($du = mysqli_fetch_array($due_)){
        ...
        $interest = $total_repayable_amount - $amnt;
        $principle_paid = $total_repa - $interest;
        if($principle_paid > $amnt){
            $principle_paid = $amnt;
        }

        $branch_principle_due_array = obj_add($branch_principle_due_array, $bran_, $amnt);
        $branch_paid_array = obj_add($branch_paid_array, $bran_, $principle_paid);
