...
while($aa = mysqli_fetch_array($all_addons)){
    ...
    elseif (in_array($addon_id, [3, 4, 5, 6])){
        ////---All penalties
...
$loans_d = fetchtable('o_loans',"disbursed=1 AND status!=0 AND given_date >= '$start_date' AND given_date <= '$end_date' $andbranch_loan","uid","asc","100000000000","uid, loan_amount, given_date, total_repaid, total_addons, status ,total_repayable_amount, loan_balance, current_branch");
...
echo "<thead><tr><th>Branch</th><th>Principal</th><th>Interest</th><th>Membership</th><th>Insurance Fee</th><th>Penalties</th><th>Total Repayable</th><th>Loan Payments</th><th>Defaulted Amount</th><th>Collection Rates</th><th>~Interest Rate</th></tr></thead>";
?>
<tbody>
<?php
...
foreach ($branches as $bid => $bname) {
    ...
    $penalty_amount = (($penalty_totals_array[$bid]));
    ...
    echo "<tr><td>$bname</td><td>".number_format($amt)."</td><td>".number_format($int_amount)."</td><td>".number_format($membership_totals)."</td><td>".number_format($processing_amount)."</td><td>".number_format($penalty_amount)."</td><td>".number_format($total_rep)."</td><td>".number_format($repaid_pi)."</td><td>".number_format($defaulted)."</td><td>$crate%</td><td>$interest_rate%</td></tr>";
...
echo "<tfoot><tr><th>Total</th><th>".number_format($amt_t)."</th><th>".number_format($int_amount_t)."</th><th>".number_format($membership_totals_t)."</th><th>".number_format($processing_amount_t)."</th><th>".number_format($penalty_amount_t)."</th><th>".number_format($total_rep_t)."</th><th>".number_format($repaid_pi_t)."</th><th>".number_format($defaulted_t)."</th><th>$crate_a%</th><th>$interest_rate_a%</th></tr></tfoot>";
...
echo "*Total Repayable: Principal + Interest + Insurance Fee + Membership + Penalties<br/>";
...
echo "*Insurance Fee: The amount that is paid for insurance<br/>";
echo "*Defaulted Amount: The amount that is in default. This is the amount that is not paid<br/>";
