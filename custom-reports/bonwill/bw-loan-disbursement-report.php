...
if ($branch > 0) {
    ...
    $andloanbranch = " AND current_branch = $branch";
} else {
    $andloanbranch = "";
}


$loans_arr = table_to_array('o_loans', "status IN (3, 4, 5, 7, 8, 9, 10, 11) AND given_date BETWEEN '$start_date' AND '$end_date' $andloanbranch", "10000000", "uid");
...
if (count($loans_arr) > 0) {
    ...
    $sql = "SELECT l.uid, c.full_name, b.name as Branch, l.account_number as Phone, l.disbursed_amount as Amount_Disbursed, l.total_addons as Interest, l.total_repayable_amount as Amount_Repayable, l.total_repaid as Amount_Repaid, l.loan_balance as Balance, l.given_date as Disbursement_Date, 
    l.final_due_date as Repayment_Date, ls.name as Status FROM o_loans l left join o_customers c on c.uid = l.customer_id left join o_loan_statuses ls on l.status = ls.uid left join o_branches b on l.current_branch = b.uid where l.uid IN($loans_string) LIMIT 1000000";
