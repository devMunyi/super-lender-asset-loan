...
<div class="col-md-6 well well-sm">
    ...
    for ($i = 1; $i <= $months_count; ++$i) {
        echo "<button class='btn btn-outline-black' onclick='filter_collec_report($i)'>" . month_name($i) . "</button> ";
    }
    if (isset($_GET['m'])) {
        $m = $_GET['m'];
    } else {
    ...
    <table class="table table-striped table-bordered">
        ...
        <tr>
            <th>Total</th>
            <th><?php echo money($total_principle_due) ?></th>
            <th><?php echo money($total_paid_) ?></th>
...
<script>
    function filter_collec_report(month) {
        ...
        const url = "?hreport=sl-branch-performance-summaries.php&m=" + month + "&y=" + year + "&from=" + from + "&to="+ to + "&branch=" + branch;
