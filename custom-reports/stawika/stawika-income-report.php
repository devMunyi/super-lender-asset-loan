...
$loan_obj = array();
$all_loans_obj = array();
$loans = fetchtable('o_loans',"disbursed=1 AND given_date BETWEEN '$from_' AND '$to_' AND status!=0","uid","asc","100000","uid, customer_id, given_date, final_due_date, account_number, product_id, loan_amount, disbursed_amount, total_repayable_amount, total_repaid, loan_balance, total_addons, transaction_code, status");
while($l = mysqli_fetch_array($loans)){

    $uid = $l['uid'];
    $customer_id = $l['customer_id'];
    $account_number = $l['account_number'];
    $product_id = $l['product_id'];  $product_name = $products[$product_id];
    ...
    $status = $l['status'];
     ...
     $loan_obj['product_name'] = $product_name;
...
echo "<table class='table table-condensed table-striped' id='example2'>";
echo "<thead><tr><th>Loan</th><th>Phone</th><th>Given Date</th><th>Due Date</th>  <th>Scheduled Penalty</th> <th>Paid Penalty </th><th>Penalty Balance</th><th>Scheduled Interest</th><th>Paid Interest</th><th>Interest Balance</th> <th>Principal</th> <th>Principal Paid</th> <th>Principal Balance</th><th>Total Repaid</th><th>Total Loan Balance</th><th>Product</th><th>Status</th></tr></thead><tbody>";
  ...
  for($i=0; $i<=sizeof($loans_array); ++$i) {
      ...
      $product_name = $all_loans_obj[$loan_id]['product_name'];
    ...
    // echo "[$kitty, $late_fee_charged, $interest_charged, $loan_amount]";
      ...
      echo "<tr><td>".$loan_id."</td><td>".$all_loans_obj[$loan_id]['account_number']."</td><td>".$all_loans_obj[$loan_id]['given_date']."</td><td>".$all_loans_obj[$loan_id]['final_due_date']."</td>  <td>".false_zero($late_fee_charged)."</td> <td>".false_zero($penalty_balance)."</td><td>".false_zero($paid_penalty)."</td><td>".false_zero($interest_charged)."</td><td>".false_zero($paid_interest)."</td><td>".false_zero($interest_balance)."</td> <td>".false_zero($loan_amount)."</td> <td>".false_zero($paid_principal)."</td> <td>".false_zero($principal_balance)."</td><td>".false_zero($total_repaid)."</td><td>".false_zero($loan_balance)."</td><td>$product_name</td><td>$loan_status</td></tr>";
      $kitty = 0;
  }
