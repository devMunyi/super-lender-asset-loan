...
// $overpayment_ = abs(fetchrow('o_loans', "uid='$loan_id'", "loan_balance"));
$overpayment_ = abs(fetchrow('o_loans', "uid='$loan_id'", "loan_balance"));
$total_addons = totaltable('o_loan_addons', "loan_id='$loan_id' AND status=1", "addon_amount");
$loan_details = fetchonerow('o_loans', "uid='$loan_id'", "loan_amount, total_repaid");
$loan_amount = $loan_details['loan_amount'] ?? 0;
$total_repaid = $loan_details['total_repaid'] ?? 0;
$total_repayable = $total_addons + $loan_amount;
$diff = $overpayment_ = $total_repayable - $total_repaid;

if ($diff > 0){
    exit("No overpayment to split");
}else {
    $overpayment_ = abs($overpayment_);
}

// echo "total_repayable: $total_repayable, total_repaid: $total_repaid, overpayment_: $overpayment_";

// check if user has uncleared loans
$uncleared_loans = intval(totaltable('o_loans', "customer_id = $customer_id AND paid = 0 AND disbursed = 1", "uid"));
...
if (strpos($string, '-') !== false) {
    die(errormes("This payment can't be split because it is already splitted"));
}

echo "<h4>Maximum Allowed Split Amount: <b>" . money($overpayment_) . "</b></h4>";
...
<form class="form-horizontal" method="POST" onsubmit="return false;">
    <div class="box-body">
        <div class="form-group">
            <label for="amount_" class="col-sm-3 control-label">Split Amount</label>

            <div class="col-sm-9">
                <input class="form-control" type="number" name="amount" id="amount_" value="<?php echo $overpayment_; ?>">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input class="form-control" type="hidden" value="<?php echo $loan_id; ?>" name="current_loan_id" id="current_loan_id">
        ...
        <div class="col-sm-9">
            <div class="box-footer">
                ...
                <button type="submit" class="btn btn-success btn-lg pull-right" onclick="split_individual_pay('overpayment');">Split </button>
            </div>
        </div>
