...
function send_money2($msisdn, $amount, $loan_id=0){
    ...
    if((input_length($server2, 5)) == 0){
        $server = 'https://www.superlender.co.ke/juhudibora';
    }
    else{
        $server = $server2;
    }
   ...
   // echo $server;
    ...
    $PartyB = $msisdn;
    $Remarks = "L$loan_id";
    $QueueTimeOutURL = $server.'/extensions/juhudi-b2c-notice.php?r='.$loan_id;
    $ResultURL = $server.'/extensions/juhudi-b2c-notice.php?r='.$loan_id;
    $Occasion = '';
