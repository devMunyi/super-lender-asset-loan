...
include_once '../configs/20200902.php';
    ...
    $today = $date;
    $day_15 = datesub($date, 0 , 0, 13);
    $day_30 = datesub($date, 0, 0,30);

    $all_loans = table_to_array('o_loans',"status!=0 AND disbursed=1 AND paid = 0","100000","uid");
  ...
  // echo $all_loans_string."<br/>";
   ...
   //die();
    ...
    $all_daily_addons = table_to_array('o_loan_addons',"addon_id=2 AND status=1 AND loan_id in ($all_loans_string)","100000000","loan_id");
    $already_added_string = implode(',', $all_daily_addons);
    $addon_d = fetchrow('o_addons',"uid=2","amount");
        ...
        if($days_ago > 12){
          ...
          //  echo "$uid: $interest <br/>";
            ...
            if(in_array($uid, $all_daily_addons) == 1){
                ...
                $update = addon_with_amount_update(2, $uid, $interest, false);
                $addon_update += 1;
                //---Update loan details to avoid recalc
            }
            else{
               ///-----Create new addons
                $addon = addon_with_amount(2, $uid, $interest, 0, false);
                $addon_add += 1;
                ///-----Update loan details
...
while ($ll = mysqli_fetch_array($loans_f)) {
   ...
   // echo "$loan_uid: $total_repayable: $loan_balance <br/>";
    ...
    $update = updatedb('o_loans',"total_repayable_amount='$total_repayable', total_addons='$total_addons',loan_balance='$loan_balance'","uid='$loan_uid'");
