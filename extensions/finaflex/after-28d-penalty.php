...
$mark = updatedb('o_loans', "status = 7", "paid = 0 AND disbursed = 1 AND final_due_date <= '$yesterday' AND status = 3 AND product_id = 5");
...
$loans = fetchtable('o_loans', "status!=0 AND disbursed=1 AND product_id = 5 AND final_due_date <= '$yesterday' AND  paid = 0", "uid", "asc", "10000000", "uid, given_date, $addon_on");
...
$addon_exists = table_to_array('o_loan_addons', "loan_id in ($all_loans_string) AND status = 1 AND addon_id = 8", "1000000", "loan_id");

for ($i = 0; $i < sizeof($all_loans); ++$i) {


    $addon_amount = ($amount / 100) * $addon_on_array[$all_loans[$i]];
