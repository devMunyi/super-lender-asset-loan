...
$customer_det = fetchonerow('o_customers',"primary_mobile='$phone_number'","uid, full_name, pin_, loan_limit, primary_product, total_loans, national_id, date(added_date) as join_date");
   ...
   if($last_choice == 1) {
       if ($text > 50 && $text <= $loan_limit) {
           ...
           if ($res >= 1) {
               ...
               //---Update 2 week loan due date
               if($text >= 2500){
                   $due_date = dateadd($date, 0, 0, 14);
                   $upd = updatedb('o_loans',"final_due_date='$due_date' ","uid='$res'");
               }
