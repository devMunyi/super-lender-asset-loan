...
if($session_d['uid'] < 1){
    ...
    $last_loan_ = fetchmaxid('o_loans',"customer_id='".$session_d['customer_id']."' AND disbursed=1 AND status != 0","given_date");
    $last_loan_date = $last_loan_['given_date'];
    $ago = intval(datediff3($last_loan_date, $date));
    ...
    if($ago > 180){
     //   $cust_details['loan_limit'] = 0;
    }

    $sec_data = $cust_details['sec_data'];
    $sec_data_obj = json_decode($sec_data, true);
    $keen_phone_ok = validate_phone(make_phone_valid($sec_data_obj['3']));

    ///----Has taken at least one loan
    $has_previous_loan = checkrowexists('o_loans',"customer_id='".$session_d['customer_id']."' AND disbursed=1 AND paid=1 AND status != 0");


    if($keen_phone_ok == 0 && $has_previous_loan == 0){
        $result_ = 0;
        $details_ = '"Profile incomplete."';
        $result_code = 114;
        echo json_encode("{\"result_\":$result_,\"details_\":$details_, \"result_code\":$result_code}");
        die();
        exit();
    ...
    $loan_det = fetchonerow('o_loans',"customer_id='".$session_d['customer_id']."' AND disbursed=0 AND paid=0 AND status  in (1, 2) ","uid, loan_amount");
    if($loan_det['uid'] > 0){
        $result_ = 0;
