...
if($session_d['uid'] < 1){
   ...
   // $primary_product = $cust_det['primary_product'];
    $primary_product = $product_id; ////We pick the selected product rather than the default product ID
    ...
    $has_loan = checkrowexists('o_loans',"customer_id='".$session_d['customer_id']."' AND disbursed=1 AND paid=0 AND status in (3, 4, 7 ,9)");
    if($has_loan == 1){
        $result_ = 0;
    ...
    $ref = "";
    $has_ref = 0;
    if((input_available($referrer_phone)) == 1){
        $referrer_phone = make_phone_valid($referrer_phone);
        if((validate_phone($referrer_phone)) == 1){
            ////------Referrer phone is valid
            $ref = array("Referred By"=>  make_phone_valid($referrer_phone));
            if($cust_det['total_loans'] < 1) {
                $has_ref = 1;
            }
            ...
            $result_ = 0;
            $details_ = '"The reference phone is invalid"';
            $result_code = 107;
            echo json_encode("{\"result_\":$result_,\"details_\":$details_, \"result_code\":$result_code}");
            store_event('o_customers', $session_d['customer_id'],"$result_, $details_, $result_code");
            die();
    ...
    if($has_ref == 1){
        //  array_push($fds, "other_info");
        //  array_push($vals, "".json_encode($ref, true)."");
    ...
    if ($create == 1) {
        ...
        $addons = fetchtable('o_product_addons',"product_id='$primary_product' AND status=1","addon_id","asc","20","addon_id");
        ...
        ///--------Check if there is a promocode for user
        $promo = fetchonerow('o_promo_codes',"promocode='$primary_mobile' AND status = 1","uid, amount_");
        if($promo['uid'] > 0){
            //---User has a promocode,
            deduction_with_amount(1, $latest_loan['uid'], $promo['amount_'], 0, false);
            ///---Add a deduction
            updatedb('o_promo_codes',"redeemed_date='$fulldate', status=2","promocode='$primary_mobile' AND status = 1");
        }
        ///
        ///---------
        //--Check end
        ...
        if($automatic_disburse == 1) {
            ...
            if($total_loans >= 1 && $recent_loan == 0 && $amount < 50000 ){
        ...
        if($has_ref == 1 ){
            ...
            $total_addons = fetchrow('o_loans',"uid=".$latest_loan['uid'], "total_addons");
            $slice = $total_addons*0.1;
            $pfds = array('campaign_id','promocode','tbl_','fld_','added_date','amount_','comments_','status');
            $pvals = array("0","$referrer_phone","o_loans",$latest_loan['uid'],"$fulldate","$slice","Referral amount for client to claim","1");
            $save_promo = addtodb('o_promo_codes',$pfds, $pvals);
        }
