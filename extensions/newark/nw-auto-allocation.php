...
$loan_branches =  array();
$loans = fetchtable('o_loans', "disbursed=1 AND paid=0 AND loan_balance > 5 AND  final_due_date < '$date'", "uid", "asc", "100000000000", "uid, final_due_date, current_agent, loan_balance, current_branch");
while ($l = mysqli_fetch_array($loans)) {
  ...
  */
    ...
    elseif ($ago >= 65 and $ago <= 150) {
        ...
        if (!in_array($current_agent, $IDC_agents)) {
...
if (isset($_GET['CC'])) {
    ...
    $agent_branches = fetchtable('o_staff_branches', "status=1 AND agent in ($CC_agents_list)", "uid", "asc", "1000000", "agent, branch");
    while ($ab = mysqli_fetch_array($agent_branches)) {
        $agent_uid = $ab['agent'];
        $branch_uid = $ab['branch'];

        $alloc_amount = false_zero($CC_allocations[$agent_uid]); // gotten from somewhere
    ...
    for ($i = 0; $i < sizeof($CC_loans); ++$i) {
        ...
        $CC_branch_allocations = $branch_agents[$loan_branch];

        $minValue = min($CC_branch_allocations); // Get the minimum value from the array
        $minAgent = array_search($minValue, $CC_branch_allocations); // Find the agent with least value
        /////---------------Save allocation to database, add logs
        /////---------------Update allocation total
