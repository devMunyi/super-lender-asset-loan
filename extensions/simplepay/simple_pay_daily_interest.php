    ...
    if ($company_d['uid'] > 0) {
      ...
      ///////-------------------------End of get company details
        ...
        if($to_day > $from_day){
         ...
         /*  $loans_to_recalc = implode(',', $loans_to_recalc_array);

          ///----All addons
          $loan_addons_array = array();
          $adds = fetchtable('o_Loan_addons',"status=1 AND loan_id in ($loans_to_recalc)","uid","asc","100000000","uid, loan_id, addon_amount");
          ...
          ////----Loop through loans
          $loans_l = fetchtable('o_loans',"uid in ($loans_to_recalc)","uid","asc","10000000","uid, loan_amount, total_repaid");
          while($ll = mysqli_fetch_array($loans_l)){
            
            $loan_id = $ll['uid'];
            $loan_amount = $ll['loan_amount'];
            $total_repaid = $ll['total_repaid'];
            echo "Recalculating $loan_id <br/>";
