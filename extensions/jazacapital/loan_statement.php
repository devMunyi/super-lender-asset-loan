...
$loan_components = array(
    ...
    'Processing Waiver' => array(), // should be -ve value and applied_date
);
...
try {
    ...
    $deductionSQL = "SELECT deduction_id, deduction_amount, DATE(added_date) AS added_date FROM o_loan_deductions WHERE loan_id = $loan_id AND `status` = 1 ORDER BY added_date ASC";
    ...
    while ($deduction = mysqli_fetch_assoc($deductionResult)) {
        ...
        $added_date = $deduction['added_date'] ?? '';

        // Processing Waiver
        if ($deduction_id == 1) {
            $loan_components['Processing Waiver'][] = array('amount' => -$deduction_amount, 'applied_date' => $added_date);
        }
    }
