...
$mark = updatedb('o_loans', "status='7'", "paid=0 AND disbursed=1 AND final_due_date <= '$yesterday' AND status=3 AND product_id=2");
...
///-------Lets start here
///------Loop through addons
$ad = fetchonerow('o_addons', "status=1 AND uid = 7", "*");
$uid = $ad['uid'];
$name = $ad['name'];
...
$loans = fetchtable('o_loans', "status!=0 AND disbursed=1 AND product_id = 2 AND final_due_date='$yesterday' AND  paid = 0", "uid", "asc", "10000000", "uid, given_date, $addon_on");
...
$addon_exists = table_to_array('o_loan_addons', "loan_id in ($all_loans_string) AND status = 1 AND addon_id = 7", "1000000", "loan_id");

for ($i = 0; $i < sizeof($all_loans); ++$i) {
    $addon_amount = $addon_on_array[$all_loans[$i]];
    if (in_array($all_loans[$i], $addon_exists)) {
        ...
        $vals = array($all_loans[$i], "7", round($addon_amount, 0), "$fulldate", "1");
