...
$sixteen_ago = datesub($date, 0, 0, 16);

echo $yesterday.$sixteen_ago;
$total = 0;
$added_ = 0;
$skipped = 0;
$loans = fetchtable('o_loans',"disbursed=1 AND paid=0 AND product_id = 2 AND status!=0 AND final_due_date BETWEEN '$sixteen_ago' AND '$yesterday'","uid","asc","100000","uid, final_due_date, loan_amount, total_repaid");
while($l = mysqli_fetch_array($loans)){
   ...
   // remove_addon(10, $uid, 1);
    ...
    $total_interest = totaltable('o_loan_addons',"loan_id='$uid' AND status=1 AND addon_id in (1,2,3,5)","addon_amount");
    $total_after_due = totaltable('o_loan_addons',"loan_id='$uid' AND status=1 AND addon_id = 6","addon_amount");
    ...
    if($total_repaid >= $total_interest+$loan_amount){
        ...
        if($interest < $total_after_due){  ////-----If we calculate a less after due, keep current afterdue
            ...
            $exists = checkrowexists('o_loan_addons', "loan_id='$uid' AND addon_id = 6 AND status=1");
            //echo
            if ($exists == 1) {
                $added = addon_with_amount_update(6, $uid, $interest, 0.7, true);
            } else {
                $added = addon_with_amount(6, $uid, $interest, 0, true);
            }
