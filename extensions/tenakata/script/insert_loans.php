...
$cust_branches = [];
$cust_phones = [];
$custs = fetchtable2('o_customers', 'uid > 0', 'uid', 'ASC', 'uid, branch, primary_mobile');

while ($c = mysqli_fetch_assoc($custs)) {
    $cust_uid = $c['uid'];
    $cust_branch = $c['branch'];
    $phone = $c['primary_mobile'];
    $cust_branches[$cust_uid] = $cust_branch;
    $cust_phones[$cust_uid] = $phone;
}
...
$disbursementsJson = file_get_contents('../data/loan_disbursements.json');
...
if (isset($paymentsData['data']) && is_array($paymentsData['data'])) {
    ...
    foreach ($paymentsData['data'] as $payData) {
        ...
        $principal_due = doubleval($payData['principal_due']);
        ...
        if (isset($expected_instal_repay_date) && $current_date >= $expected_instal_repay_date) {
            ...
            $current_instalment_amounts[$loan_id] = $due;
        ...
        if ($current_date <= $expected_instal_repay_date && !isset($next_due_dates[$loan_id])) {
            $next_due_dates[$loan_id] = $expected_instal_repay_date_r;
        }                    
        

        // final due date handler
        if ($current_date > $expected_instal_repay_date) {
            $final_due_dates[$loan_id] = $expected_instal_repay_date_r;
        } else {
        ...
        //  elseif ($current_date > $expected_instal_repay_date) {
        //             $final_due_dates[$loan_id] = $expected_instal_repay_date_r; // for final_due_date
        //         }
...
if (isset($loansData['data']) && is_array($loansData['data'])) {
    ...
    foreach ($loansData['data'] as $loanData) {
        ...
        $account_number = make_phone_valid($account_number);

        if ($account_number == '254') {
            $account_number = '';
        }
        ...
        $total_repaid = $repaid[$uid] ? $repaid[$uid] : 0;
        $period = 30;
        $period_units = 1;
        $payment_frequency = 1;
        $payment_breakdown = '';
        ...
        $total_instalments = 4; // needs clarification as I saw one that had 30 instalments on repayment schedule (loan_id: 162064)
        ...
        $given_date = $loanData['disbursement_date'] ? $loanData['disbursement_date'] : '0000-00-00';
        $status = $loanData['status'] ?? 0; // needs clarification as jisort has status not present in superlender like status with id 12(reversed)

        // if($status == 9){

        // }
        $next_due_date = $next_due_dates[$uid] ?? $final_due_dates[$uid] ?? '0000-00-00';
        $final_due_date = $final_due_dates[$uid] ?? '0000-00-00';


        $added_by = 0;
        $current_agent = 0;
        $current_branch = $cust_branches[$customer_id] ? $cust_branches[$customer_id] : 0;
        $added_date = $loanData['date_of_loan_application'] ?? '0000-00-00 00:00:00';
        $loan_stage = $loanData['status'] ?? 0; // needs clarification as jisort doesn't have such an attribute
        $loan_flag = 0;
        ...
        $application_mode = 'API'; // needs clarification
        $disburse_state = $loanData['status_name'] ?? 'Unknown';
        $disbursed = 1;


        // next due and final due overide based on status = 9
        if ($status == 9 && $given_date != '0000-00-00') {
            $next_due_date = addDaysToDate($given_date, 7);
            $final_due_date = addDaysToDate($given_date, 30);
        }
        ...
        if($total_repaid > 0 && $current_date < new DateTime($final_due_date) && $loan_balance > 0){
            $status = 4;
        }
        // 2) overdue status
        elseif($current_date > new DateTime($final_due_date) && $loan_balance > 0){
            $status = 7;
