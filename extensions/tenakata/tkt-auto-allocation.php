...
while ($l = mysqli_fetch_array($loans)) {
    ...
    $date_7_ago = datesub($date, 0, 0, 7);  ///// Subtract 7 from the date
    $is_this_month = isThisMonth($date_7_ago);  ////// Check if 7 days ago is still this month or not

    if ($is_this_month == 0) { ////----It's not this month
        ///-----FA
...
if (isset($_GET['IDC'])) {
    ...
    $fds = array('tbl', 'fld', 'event_details', 'event_date', 'event_by', 'status');
    $log = addtodbmulti('o_events', $fds, ltrim($mass_logging, ","));
    echo "Log: $log <br/> $mass_logging<br/>";
}
...
function isThisMonth($date)
{
    ...
    // Get the input date's month and year
