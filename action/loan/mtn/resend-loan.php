...
$resend_loan = permission($userd['uid'],'o_mtn_queues',"0","RESEND");
if($resend_loan != 1){
...
if($update_loan_stage == 1){
    ...
    ///-----Queue Message
    $queued = fetchonerow('o_mtn_queues',"loan_id=$loan_id","trials, status, uid");
    ...
    if($q_uid > 0){
        ...
        if($q_trials > 2){
            ...
            if($q_status == 1){
                ...
                $update_ = updatedb('o_mtn_queues',"status=1, feedbackcode='Requeued', requeued_date='$fulldate'","loan_id=$loan_id");
                if($update_ == 1){
