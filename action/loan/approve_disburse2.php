...
require("../../php_functions/rmqUtils.php"); // required by rmq.php to work
...
if ($lstatus != 1) {
    ...
    exit(notice("Loan is not approved. Current status is $status_name"));
}
...
if ($update_loan_stage == 1 && $disburse_method == 2) {
    ...
    $event = "Loan moved to disbursement by [" . $userd['name'] . "(" . $userd['email'] . ")] with comment [<i>$comment</i>]";
    ...
    if ($configs_available == 1 && $disbursed != 1) {
        ...
        if ($cc == 254) {
            ...
            if (b2CRmqIsSet()) {
                $queue_id = intval(queue_money($mobile_number, $customer_id, $disbursed_amount, decurl($loan_id),  $userd['uid']));

                if($queue_id == 1){
                    // set variables required by RMQ
                    $queueName = QueueName::B2CDEFQ;
                    $msgID = decurl($loan_id);
