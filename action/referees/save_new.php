...
if(validate_phone($mobile_no) == 0){
    ...
    $total_users = countotal('o_customer_referees',"uid > 0 AND status > 0 AND mobile_no = \"$mobile_no\"","uid");
  //  $mobile_exists = checkrowexists("o_customer_referees", "uid > 0 AND status > 0 AND mobile_no = \"$mobile_no\"");
    if($total_users > 2){
        die(errormes("Referee already in use by $total_users users"));
    }
}
...
{
    echo errormes('Unable to Add Referee'.$create);
}
