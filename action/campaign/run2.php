...
include_once __DIR__ . '/../../vendor/autoload.php'; // Adjusting the path relative to the current file
...
if ($campaign_id > 0) {
    ...
    if ($status == 1) {
        ...
        if ($valid_audience == 1) {
            ...
            $message_id = 0;
            $message = fetchonerow('o_campaign_messages', "status=1 AND campaign_id=$campaign_id", "uid, message, type");
            $message_id = $message['uid'];
            $message_body = $message['message'] ?? '';
            $message_type = trim($message['type'] ?? 'PERSONALIZED');

            //==  check if message body contains curly braces if so it's personalized messsage_type
            // ==== enforce message type based on the presence of curly braces
            if (strpos($message_body, '{') !== false && strpos($message_body, '}') !== false) {
            ...
            if ($message_id < 1) {
                ...
                $all_phones = implode(',', array_map(fn($phone) => "'$phone'", $phones_array));
                ...
                $customer_sql = "SELECT uid, full_name, loan_limit, primary_mobile FROM o_customers WHERE primary_mobile IN ($all_phones)";
                $customers = mysqli_query($con, $customer_sql);
                ...
                if ($queued_sms > 0) {
                    ...
                    if(smsRmqIsSet() && $message_type == 'GENERAL'){
                        $queueName = QueueName::CAMPAIGNSMSQ;
                        $msgID = $campaign_id;
