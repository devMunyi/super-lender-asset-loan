...
if($branch > 0){
   ...
   $unall = updatedb('o_loans',"current_lo=0, current_co=0","current_branch='$branch' AND status!=0");
   if($unall == 0){
       die(errormes("Unable to unallocate loans, please retry"));
   }
   ...
   foreach ($new_lo as $loan_id => $lo ){
       ...
       if($updatebdo == 1) {
           ...
           store_event('o_loans', $loan_id, "LOAN BDO Updated to LO:<b>".$username[$lo]."($lo)</b> AND CO: <b>". $username[$co]."($co)</b> on $fulldate by manual action");
       }
