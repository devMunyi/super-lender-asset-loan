...
$new_loan_id = intval($_POST['new_loan_id']);
$split_amt = $_POST['amount'];
...
if ($customer_idd > 0) {
    ...
    exit(errormes("Customer ID required"));
}
if ($split_amt > 0) {
} else {
...
if ($total_allocated + $split_amt > $pay_amount) {
...
$vals = array("$customer_id", "$branch_id", "$group_id", "$split_from", "$payment_method", "$payment_for", "$mobile_number", "$split_amt", "SP-$random-$transaction_code", $new_loan_id, "$loan_code", "$init_payment_date", "$record_method", "$recorded_date", "$added_by", $collected_by, "$comments", $status);


$create_split_payment = addtodb('o_incoming_payments', $fds, $vals);
if ($create_split_payment == 1) {
    ...
    echo errormes('Error Splitting Payment' . $create_split_payment);
}
