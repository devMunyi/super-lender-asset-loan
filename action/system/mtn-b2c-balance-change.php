...
$update = updatedb('o_summaries', "value_= '$balance'", "uid=4");
if($update == 1){
    echo sucmes('MTN B2C Balance Updated Successfully');
    $event = "MTN B2C Balance changed by ".$userd['name']." (".$userd['uid'].")";
    store_event('o_summaries', 1 ,"$event");
    $proceed = 1;

}else{
        echo errormes('Error Updating MTN B2C Balance');
        exit();
}
