...
// enable all errros and warnings
...
///////-------------------------End of get company details
        if ($req_id > 0 && $ResultCode == '0') {
            ...
            // store_event('o_loans', $req_id, "Error: on $fulldate , M-Pesa was unable to deliver the funds with message ($ResultDesc). Transcode ($TransactionID) ");
            ...
            if (strpos($ResultDesc, $low_balance_message) !== false && strlen($ResultDesc) > strlen($low_balance_message)) {
                ...
                // requeue the loan
               // mpesa_loan_requeue($req_id, 'Requeued');
        ...
        if ($B2CUtilityAccountAvailableFunds > 0) {
            ...
            updatedb('o_summaries', "value_='$B2CUtilityAccountAvailableFunds', last_update='$fulldate'", "uid=1 $oruid");
        }
    }
} else {
