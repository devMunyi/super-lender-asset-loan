...
///////-------------------------End of get company details
       ...
       $company_name = $current_company['name'];
        ...
        $to = $_POST['to'];
        $text = trim(ltrim(strtolower($_POST['text'])), 'finaflex');
                                        ...
                                        if ($amount <= $loan_limit && $amount <= $max_amount && $amount >= $min_amount) {
                                          ...
                                          /*  $given_date = $date;         ////Initialization
                                            ...
                                            if ($create == 1) {
                                                ...
                                                $feedback = "Send Money Response".$latest_loan['uid'].send_money($from, $amount, $latest_loan['uid']);
                                            } else {
    ...
    if((input_length($feedback, 3)) == 1){
        ////----SEnd Feedback
        echo $feedback;
         $res = send_sms_interactive($from, $feedback, $linkId);
          $hello = send_sms_bulk($from,  "$feedback")."<br/>";
        $logFile = 'sms-log.txt';
        $log = fopen($logFile,"a");
        fwrite($log, $data.'Bulk-Result-'.$hello."-> $log ->".date('Y-m-d H:i:s')."\n");
        fclose($log);
