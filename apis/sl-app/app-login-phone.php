...
if($proceed === 1){
    ...
    $vals = array($user_['uid'], "$device_id", "$session_code", "$starting", "$ending", 8);
    $start = addtodb('o_customer_sessions',$fds, $vals);
    $customer_details = array();
    $customer_details['phone_'] = $user_['primary_mobile'];
    $customer_details['full_name'] = $user_['full_name'];
    $customer_details['session_code'] = $session_code;


    if($start == 1){
        $result_ = 1;
        $result_code = 111;
        $details_ = json_encode($customer_details);
    }
    else{
        $result_ = 0;
        $result_code = 106;
