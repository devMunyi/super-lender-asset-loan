...
$apply_amount = (trim($data['apply_amount']));
...
if($proceed === 1){
    ...
    ///---Use imternal loan application process
    $give_loan = give_loan($cust_id, $primary_product, $amount, 'APP');
    if($give_loan > 10){
        $result_ = 1;
        ...
        $result_ = 0;
        $details_ = '"An error occurred. '.$give_loan.'"';
        $result_code = 105;
        echo json_encode("{\"result_\":$result_,\"details_\":$details_, \"result_code\":$result_code}");
    }
    ...
    if ($create == 1) {
       ...
       // mpesa_addon(4, $latest_loan['uid'], false);
        ...
        if($automatic_disburse == 1) {
            ...
            ///------- Security checks.     Send money if its qualified
            ///------Taken loan the last 60 days
            ///------Not taken loan the last 3 days
            ///------
            ...
            $vals = array(make_phone_valid($primary_mobile),"Dear customer, We have received your request. Please be patient as we review the request. We typically take less than 30 minutes.", "$fulldate","$fulldate","1","1");
