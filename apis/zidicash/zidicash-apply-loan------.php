        ...
        if($automatic_disburse == 1) {
           // send_money($primary_mobile, $amount, $latest_loan['uid']);
            ...
            if($total_loans >= 1 AND $recent_loan == 0){
                ...
                $note = "Dear Customer, we have received your request and its being processed. If you dont receive the funds within 5 minutes, contact us via: 0714439868";
                queue_message("$note",make_phone_valid($primary_mobile), $cust_id, 'o_loans', $recent_loan);
            }
            else{
                ...
                $vals = array(make_phone_valid($primary_mobile),"Dear customer, We have received your request. Please be patient as we review the request. We typically take less than 30 minutes. In case it takes longer Contact us on 0714439868", "$fulldate","$fulldate","1","1");
                $save_ = addtodb('o_sms_outgoing', $fds, $vals);

                $fds = array('phone','message_body','queued_date','sent_date','created_by','status');
                $vals = array(254702332796,"Please check the new loan that requires your approval: $primary_mobile", "$fulldate","$fulldate","1","1");
                $save_ = addtodb('o_sms_outgoing', $fds, $vals);
