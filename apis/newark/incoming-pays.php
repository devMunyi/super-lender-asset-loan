...
if ($TransAmount > 0) {
    ...
    if ($customer_id == 0) {
        ...
        if ($msisdnHasConsecutiveAsterisks) {
            ...
            $name_ = trim($name);
            $customer_det = fetchonerow('o_customers', "enc_phone LIKE '$MSISDN_' AND status != 0 AND full_name LIKE '%$name_%'", "uid, branch, primary_mobile");
            ...
            $customer_det_query = "SELECT customer_id as uid, current_branch as branch, account_number as primary_mobile FROM o_loans WHERE enc_phone = '$MSISDN' AND disbursed = 1 order by uid desc limit 1";
