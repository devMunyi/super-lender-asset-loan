...
//-----------------------------Reused Query
$o_notifs = fetchtable('o_notifications',"$where_ AND status > 0 AND staff_id = $staff_id $andsearch", "$orderby", "$dir", "$limit", "*");
///----------Paging Option
$alltotal = countotal("o_notifications","$where_ AND status > 0 AND staff_id = $staff_id $andsearch");
///==========Paging Option
...
if($alltotal > 0) {
    while ($k = mysqli_fetch_array($o_notifs)) {
        ...
        $row .= "<li><a href='#' class='text-black'> <i class='fa fa-bell-o'></i> <span class='font-16 font-bold'>$title</span> <span class='pull-right text-muted'> $sent_date</span> <br/> $details  </a></li>";
