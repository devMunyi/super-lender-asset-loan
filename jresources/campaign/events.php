...
if ($campaign_id == 0) {
    echo "<div class='row'><span class='font-18 font-italic text-black text-mute'>Campaign id was not parsed!</span></div>";
    exit();
} else {
    $campaign_id = decurl($campaign_id);
}

$view_events = permission($userd['uid'], 'o_events', "0", "read_");
if ($view_events == 1) {

    $o_events_ = fetchtable('o_events', "tbl='o_campaigns' AND fld=$campaign_id AND status = 1", "uid", "asc", "100", "uid, event_details, event_date ");
