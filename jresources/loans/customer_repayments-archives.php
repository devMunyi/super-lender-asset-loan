...
$andloan = $andcustomer = "";
if($customer_id > 0){
    $andcustomer = " AND customer_id='".decurl($customer_id)."'";
}
if($loan_id > 0){
...
?>
    <h4 class='text-orange'>PAYMENTS FROM ARCHIVES</h4>
<div class="well well-sm">

<table class="table-bordered table-striped font-14 table table-hover table-condensed">
    <thead><tr><th>ID</th><th>Transaction Code</th><th>Amount</th><th>Date Repaid</th><th>Payer Details</th><th>Loan ID</th><th>Status</th><th>Action</th></tr></thead>
    <tbody>
    <?php
    //-----------------------------Reused Query
    $o_pays_ = fetchtable_archive('o_incoming_payments', "status = 1 $andcustomer $andloan", "recorded_date", "desc", "0,100", "*");



    ///----------Paging Option
    $alltotal = countotal_archive("o_incoming_payments", "status = 1 $andcustomer $andloan","uid","1000");
    ///==========Paging Option
    if ($alltotal > 0) {
        ...
        echo "<tr><td colspan='13'><i>No Records Found from archives</i></td></tr>";
    }                                                                                           
    ?>
    </tbody>
