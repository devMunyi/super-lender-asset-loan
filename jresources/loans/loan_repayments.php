...
<table class="table-bordered font-14 table table-hover">
    ...
    <?php
    //-----------------------------Reused Query
    $o_pays_ = fetchtable('o_incoming_payments', "loan_id=".decurl($loan_id)." AND status = 1", "uid", "desc", "0,100", "*");
    ///----------Paging Option
    $alltotal = countotal("o_incoming_payments", "loan_id=".decurl($loan_id)." AND status = 1");
    ///==========Paging Option
    if ($alltotal > 0) {
