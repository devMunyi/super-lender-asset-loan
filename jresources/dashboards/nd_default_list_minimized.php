...
$loan_q = "status!=0 AND disbursed=1 AND paid = 0 AND final_due_date >= '$start_date' AND final_due_date <= '$end_date' AND disbursed=1 $loan_branches";
...
$loans = fetchtable('o_loans',"uid in ($loan_list)","uid","asc","1000000000","uid, given_date, customer_id,loan_amount, product_id ,status, total_repaid, current_branch ,loan_balance, paid, total_repayable_amount, current_lo, current_co,group_id, group_id, loan_balance, status, other_info, total_addons, total_deductions");
while($l = mysqli_fetch_array($loans)) {
   ...
   // $principal_array[$obj_id] = $principal;
    ...
    $total_accounts_array = obj_add($total_accounts_array, $obj_id, 1);
...
<table class="tablex">
    ...
    <tr><th><?php echo $obj; ?></th><th>PRINCIPLE</th><th>INTEREST</th><th>OTHER CHARGES</th><th>REPAYABLE</th><th>PAID</th><th>BALANCE</th><th>ACCOUNTS</th></tr>
    </thead>
    <tbody>
    <?php
    $obj_ = array();
    foreach ($obj_array as $obj_id => $obj_name ){
       ...
       $accounts = $total_accounts_array[$obj_id]; $accounts_total+=$accounts;
    ...
    <tfoot>
    <?php
    echo " <tr class='bg-gray text-bold'><td>TOTAL</td><td>".number_format($principal_total)."</td><td class='text-blue'>".number_format($interest_total)."</td><td>".number_format($other_charges_total)."</td><td>".number_format($total_repayable_total)." </td><td>".number_format($total_total_repaid)."</td><td class='text-red'>".number_format($total_balance_total)."</td><td class='font-italic'>$accounts_total</td></tr>";
