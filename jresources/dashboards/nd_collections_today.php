...
while($l = mysqli_fetch_array($loans)){
    ...
    if(datediff3($final_due_date, $date) == 0){
    ...
    elseif ((datediff3($final_due_date, $date) == 1) || ($loan_status == 7)){
        ///----Overdue yesterday
...
if($total_repaid_today > $total_repaid_yest){
    $today_analysis = "<span class='text-green'><i class=\"fa fa-thumbs-up\"></i> Better Collection that Yesterday</span>";
}
elseif ($total_repaid_today < $total_repaid_yest){
    $today_analysis = "<span class='text-red'><i class=\"fa fa-warning\"></i> Lower collection rate than yesterday</span>";
}
...
<table class="table font-16 table-hover table-bordered">
    ...
    <tr class="text-red font-bold"><td>Overdue</td><td colspan="3"> <?php echo number_format($overdue_repayable_today); ?> <span class="label bg-danger
                            text-black"><?php echo $overdue_repayable_today_number;  ?> Loans</span> <i class="font-400">(Due Yesterday)</i></td></tr>
