...
if (isset($_GET['starting_date'])) {
    ...
    $and_starting_date = " AND given_date >= '$starting_date'";
} else {
    $and_starting_date = "";
}
...
if ($addonId > 0) { {
        ...
        if ($loan_stage == 'FINAL_DEFAULT') {
            $and_status = " AND status=7";
        }
        ...
        echo "anddate $anddate \n";
        $loan_total_days_array = array();
        $loans = fetchtable('o_loans', "status!=0 AND product_id in ($prod_addons_string) $anddate $and_starting_date AND disbursed=1 AND paid = 0 $and_status", "uid", "asc", "100000000", "uid, given_date, $addon_on, final_due_date");
        while ($l = mysqli_fetch_array($loans)) {
            ...
            // ===== Begin handle skip of sunday dues
            $date_to_check = new DateTime($final_due_date);

            // subtract the specified number of days
            $date_to_check->modify("-1 days");

            // the following variable is added to the config file for flexibility
            // $sunday_addons_exmptions = [
            //     4, // DD+1
            // ]; // for tenakata

            // Check if the resulting day is Sunday (0 in the format 'w')
            $period_count = datediff($given_date, $final_due_date); // exempt 31 days due to sunday coincidence wit the usual expected final due date
            if ($ignore_date != 1 && $date_to_check->format('w') === '0' && in_array($addonId, $sunday_addons_exmptions) && $period_count == 31) {
                // skip this loan
                continue;
            }
            // ===== End handle skip of sunday dues
        ...
        for ($i = 0; $i < sizeof($all_loans); ++$i) {
            ...
            echo "[[[Factor:$mult_factor,Loan:" . $all_loans[$i] . ", Amnt: $amountx, Addon:$addon_amount]]]<br/>";
