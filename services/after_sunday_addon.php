...
if ($company > 0 && $addonId > 0) {
    ...
    if ($company_d['uid'] > 0) {
        ...
        $loans = fetchtable('o_loans', "status!=0 AND disbursed=1 AND product_id in ($prod_addons_string) $anddate AND disbursed=1 AND paid = 0", "uid", "asc", "100000000", "uid, given_date, $addon_on, final_due_date");
        while ($l = mysqli_fetch_array($loans)) {
            ...
            // ===== handle only loans whose due was on Sunday
            ...
            if ($date_to_check->format('w') === '0' && in_array($addonId, $sunday_addons_exmptions)) {
                
                // check if addon is already applied 
                $addon_exists = table_to_array('o_loan_addons', "loan_id in ($uid) AND status=1 AND addon_id='$addonId'", "1000000", "loan_id");
                if (in_array($uid, $addon_exists)) {
                    // skip this loan
                    continue;
                } else {
                    // echo "DUE NOT IN ARRAY: $uid, $given_date \n";
                    ////-----Calculate Interests
